...
class EverTrackPopup {
  ...
  async loadData() {
    try {
      ...
      this.timeData = await EverTrackAPI.fetchTimeData(this.settings.apiToken, this.settings.trackingMode);
      console.log('EverTrack Popup: Time data received:', this.timeData);

      // Show content and update display
      EverTrackDOM.showContent(this.elements.content, this.elements.loading, this.elements.error);
  ...
  updateDisplay() {
    ...
    // Calculate progress
    const progress = EverTrackTime.calculateProgress(
      this.timeData,
      targetHours,
    ...
    EverTrackDOM.setText(
      this.elements.modeLabel,
      `${this.settings.trackingMode} (${periodDescription}) target ${targetHours}h`,
    );
  }
   ...
   */
  showError(message) {
    EverTrackDOM.showError(this.elements.error, message, this.elements.content);
